...
impl<'a, T> Default for BiSlice<'a, T> {
    #[inline(always)]
    fn default() -> Self {
        Self {
...
impl<'a, T: fmt::Debug> fmt::Debug for BiSlice<'a, T> {
    #[inline]
...
impl<'a, T> Clone for BiSlice<'a, T> {
    #[inline(always)]
    fn clone(&self) -> Self {
        Self {
            start: self.start,
...
impl<'a, T> ops::Index<usize> for BiSlice<'a, T> {
    type Output = T;
...
impl<'a, T> Iter<'a, T> {
    ...
    #[inline(always)]
    fn from_slice(slice: BiSlice<'a, T>) -> Self {
...
impl<'a, T> DoubleSliceMut<'a, T> {
    ...
    #[inline(always)]
    #[must_use]
    pub fn copy_from_slice(self, source: BiSlice<'a, T>) -> Option<()>
    where
        T: Copy,
    {
    ...
    #[inline(always)]
    #[must_use]
    pub fn clone_from_slice(self, source: BiSlice<'a, T>) -> Option<()>
    where
        T: Clone,
    {
        if self.reborrow().len() != source.len() {
...
impl<'a, T> DoubleSliceMut<'a, mem::MaybeUninit<T>> {
    #[inline(always)]
    #[must_use]
    pub fn write_copy_of_slice(self, slice: BiSlice<'_, T>) -> Option<DoubleSliceMut<'a, T>>
    where
        T: Copy,
    {
    ...
    #[inline(always)]
    #[must_use]
    pub fn write_clone_of_slice(self, slice: BiSlice<'_, T>) -> Option<DoubleSliceMut<'a, T>>
    where
        T: Clone,
    {
        if self.reborrow().len() != slice.len() {
